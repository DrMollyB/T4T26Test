...
const sheetId = "13ll3AmFDpuBUBb-ujlRdpsYTAjSDAUuDOC8EAyCdvN4";
...
let detailsByPresenter = new Map();
...
function normalizePresenterLookup(value) {
  return normalizeLookup(String(value || "").replace(/\([^)]*\)/g, ""))
...
function detailKey(title, presenter = "") {
  return `${normalizeLookup(title)}|${normalizePresenterLookup(presenter)}`;
}

function getSessionDetail(title, presenter = "") {
  const exactMatch = detailsByTitlePresenter.get(detailKey(title, presenter));
  if (exactMatch) return exactMatch;

  const titleMatch = detailsByTitle.get(normalizeLookup(title));
  if (titleMatch) return titleMatch;

  const presenterMatches = detailsByPresenter.get(normalizePresenterLookup(presenter)) || [];
  const normalizedTitle = normalizeLookup(title);
  const closePresenterMatch = presenterMatches.find((detail) => {
    const detailTitle = normalizeLookup(detail.title);
    return detailTitle && (detailTitle.includes(normalizedTitle) || normalizedTitle.includes(detailTitle));
  });
  if (closePresenterMatch) return closePresenterMatch;

  const presenterOnlyMatches = presenterMatches.filter((detail) => !detail.title);
  if (presenterOnlyMatches.length === 1) return presenterOnlyMatches[0];
...
const operationSearchResults = document.querySelector("#operationSearchResults");
const operationResultCount = document.querySelector("#operationResultCount");
...
const translations = {
  en: {
    ...
    operationSearchPlaceholder: "Search sessions, presenters, rooms, or keywords...",
  ...
  es: {
    ...
    operationSearchPlaceholder: "Buscar sesiones, presentadores, salones o palabras clave...",
  ...
  fr: {
    ...
    operationSearchPlaceholder: "Rechercher des séances, intervenants, salles ou mots-clés...",
  ...
  hi: {
    ...
    operationSearchPlaceholder: "सत्र, प्रस्तुतकर्ता, कमरे या कीवर्ड खोजें...",
...
function getSheetHeadersAndRows(response) {
  const tableRows = response.table?.rows || [];
  let headers = (response.table?.cols || []).map((column) => (column.label || "").trim().toLowerCase());
  const firstRowHeaders = tableRows[0]?.c?.map((cell) => String(cell?.f || cell?.v || "").trim().toLowerCase()) || [];
  const knownHeaderWords = ["room", "speaker", "presenter", "title", "time", "description", "audience", "strategy"];
  const headerScore = headers.filter((heading) => knownHeaderWords.some((word) => heading.includes(word))).length;
  const firstRowScore = firstRowHeaders.filter((heading) => knownHeaderWords.some((word) => heading.includes(word))).length;

  if (firstRowScore > headerScore) {
    headers = firstRowHeaders;
    return { headers, tableRows: tableRows.slice(1) };
  }
...
function rowsFromSheetResponse(response) {
  const { headers, tableRows } = getSheetHeadersAndRows(response);
  const roomIndex = headers.findIndex((heading) => heading.includes("room"));
  const presenterIndex = headers.findIndex((heading) => heading.includes("speaker") || heading.includes("presenter"));
  const titleIndex = headers.findIndex((heading) => heading.includes("title"));
  const timeIndex = headers.findIndex((heading) => heading.includes("time"));
...
function rowsFromDetailsResponse(response) {
  ...
  return tableRows
    ...
    .filter((detail) => detail.presenter && (detail.title || detail.description));
}

function setSessionDetails(details) {
  detailsByTitle = new Map();
  detailsByTitlePresenter = new Map();
  detailsByPresenter = new Map();
  ...
  details.forEach((detail) => {
    ...
    if (detail.presenter) {
      const presenterKey = normalizePresenterLookup(detail.presenter);
      detailsByTitlePresenter.set(detailKey(detail.title, detail.presenter), detail);
      detailsByPresenter.set(presenterKey, [...(detailsByPresenter.get(presenterKey) || []), detail]);
    }
  });
}
...
function getCurrentViewState() {
  return {
    ...
    selectedOperationTitle,
    selectedPresenter,
    operationSearch: operationSearch.value,
...
function restoreViewState(state) {
  ...
  operationSearch.value = state.operationSearch || "";
...
function renderOperationSchedule() {
  ...
  operationSearchResults.replaceChildren();
  const query = operationSearch.value.trim();

  if (query) {
    const matchingSessions = sessions.filter((session) => sessionMatchesSearch(session, query));
    operationScheduleList.hidden = true;
    operationResultCount.textContent =
      matchingSessions.length === 1 ? t("oneResult") : t("manyResults", { count: matchingSessions.length });

    if (!matchingSessions.length) {
      const empty = document.createElement("div");
      empty.className = "empty-state";
      empty.textContent = t("noResults");
      operationSearchResults.append(empty);
      return;
    }

    matchingSessions.forEach((session) => operationSearchResults.append(createSessionCard(session)));
    return;
  }

  operationScheduleList.hidden = false;
  operationResultCount.textContent = "";
...
operationSearch.addEventListener("input", renderOperationSchedule);
